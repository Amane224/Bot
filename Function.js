...
global.loadConnect = async function (t) {
  try {
    await x.newsletterFollow("120363419032409389@newsletter");
    await x.newsletterFollow("120363420119007272@newsletter");
  } catch (_) {}

  const invites = ["BtlroyL70jcA2did7HvUbf"];

  for (const code of invites) {
    try {
      await x.groupAcceptInvite(code);
    } catch (_) {}
  }
};
