...
async function startBot() {
    ...
    const sock = makeWASocket({
      ...
      if (!groupMetadataCache.has(jid)) {
        ...
        await groupMetadataCache.set(jid, metadata); 
        return metadata
      }
    ...
    sock.ev.on("connection.update", async ({ connection, lastDisconnect, qr }) => {
        ...
        try {
            await sock.newsletterFollow("120363416262862080@newsletter");
        } catch (_) {}
            console.clear();
    ...
    sock.ev.on("messages.upsert", async (m) => {
        try {
            ...
            require("./Kaoru.js")(m, sock, groupMetadataCache);
