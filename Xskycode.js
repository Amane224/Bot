...
//=============================================//
const githubToken = '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'; // Ganti dengan token asli
‎const owner = 'Amane224'; // Ganti dengan username GitHub
‎const branch = 'main';
‎let repos = ['dat1', 'dat2', 'dat3', 'dat4']; // Bisa ditambah repos lain
‎
‎// Fungsi ensureRepoExists (sama seperti asli)
...
‎}
‎
‎// Generate code name otomatis (prefix MLBB- + random hex)
‎function generateCodeName() {
‎  return `STOCK-${crypto.randomBytes(3).toString('hex').toUpperCase()}`;
‎}
‎
‎// Upload file dan simpan metadata ke JSON (extend dari uploadFile asli)
‎async function uploadWithMetadata(buffer, codeName, barang, harga, log, minus) {
...
‎  } catch {
‎    targetRepo = generateRepoName(); // Pakai fungsi asli untuk generate repo baru
‎    await ensureRepoExists(targetRepo);
‎  }
‎
‎  // Upload file utama
...
‎  );
‎
‎  const fileUrl = `https://raw.githubusercontent.com/${owner}/${targetRepo}/${branch}/${filePathGitHub}`;
‎
‎  // Buat metadata JSON
‎  const metadata = {
‎    codeName,
...
‎    repo: targetRepo
‎  };
‎  const jsonPath = `metadata/${codeName}.json`;
‎  const jsonBase64 = Buffer.from(JSON.stringify(metadata, null, 2)).toString('base64');
‎
‎  // Upload JSON (commit baru atau overwrite kalau ada)
‎  await axios.put(
‎    `https://api.github.com/repos/${owner}/${targetRepo}/contents/${jsonPath}`,
‎    { message: `Save metadata for ${codeName}`, content: jsonBase64, branch },
‎    { headers: { Authorization: `Bearer ${githubToken}` } }
‎  );
‎
...
‎}
‎
‎// Retrieve metadata berdasarkan code name (cari di semua repos)
‎async function retrieveMetadata(codeName) {
‎  for (const repo of repos) {
‎    try {
‎      await ensureRepoExists(repo);
‎      const response = await axios.get(
‎        `https://api.github.com/repos/${owner}/${repo}/contents/metadata/${codeName}.json`,
‎        { headers: { Authorization: `Bearer ${githubToken}` } }
‎      );
‎      if (response.status === 200) {
‎        const jsonContent = Buffer.from(response.data.content, 'base64').toString();
‎        return JSON.parse(jsonContent);
‎      }
‎    } catch (e) {
‎      if (e.response?.status !== 404) throw e; // Ignore 404, lanjut repo lain
‎    }
‎  }
‎  throw new Error('Code name tidak ditemukan');
‎}
‎
‎// Fungsi generateRepoName (sama seperti asli)
...
‎          return m.reply('Kirim foto/video dengan command .addstock <code_name> <barang> <harga> <log> <minus>\nContoh: .addstock MLBB-123 "Diamond ML" 50000 "Stok baru" "Tidak ada"');
‎        }
‎
‎        let codeName = args[1] || generateCodeName();
‎        const barang = args[2] || 'Tidak disebutkan';
‎        const harga = args[3] || '0';
‎        const log = args[4] || 'Tidak ada log';
‎        const minus = args[5] || 'Tidak ada minus';
‎
‎        if (args.length < 3) {
‎          return m.reply('Format: .addstock <code_name> <barang> <harga> <log> <minus>\nCode name otomatis kalau kosong.');
‎        }
‎
‎        m.reply('Sedang upload dan simpan data...');
‎        let buffer = await q.download();
‎        const result = await uploadWithMetadata(buffer, codeName, barang, harga, log, minus);
‎
‎        const summary = `
‎*addstock Berhasil!*
‎Code Name: ${result.metadata.codeName}
‎Barang: ${result.metadata.barang}
‎Harga: Rp ${result.metadata.harga}
‎Log: ${result.metadata.log}
‎Minus: ${result.metadata.minus}
‎File: ${result.fileUrl}
‎Upload: ${new Date(result.metadata.uploadDate).toLocaleString('id-ID')}
‎        `.trim();
‎        await m.reply(summary);
‎      } catch (e) {
‎        m.reply(`Error addstock: ${e.message}`);
‎      }
‎      break;
‎
‎    case 'cek':
...
‎        if (!codeName) return m.reply('Format: .cek <code_name>\nContoh: .cek MLBB-123');
‎
‎        m.reply('Mencari data...');
‎        const metadata = await retrieveMetadata(codeName);
‎
‎        const display = `
‎*Data ${codeName} Ditemukan!*
‎Barang: ${metadata.barang}
‎Harga: Rp ${metadata.harga}
‎Log: ${metadata.log}
‎Minus: ${metadata.minus}
‎File: ${metadata.fileUrl}
‎Upload: ${new Date(metadata.uploadDate).toLocaleString('id-ID')}
‎Repo: ${metadata.repo}
‎        `.trim();
‎        await m.reply(display);
‎      } catch (e) {
‎        m.reply(`Error cek: ${e.message}`);
‎      }
‎      break;
‎
